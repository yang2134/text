...
public class removeHTML {
	...
	//html中图片自适应
	public static String processHtmlContent(String content) {
		//去掉html中所有img的宽度和高度，用正则表达式
		content = content.replaceAll("width=\"\\d*\"", "");
		content = content.replaceAll("height=\"\\d*\"", "");

		//去掉html中所有style属性，用正则表达式
		content = content.replaceAll("style=\"[^\"]+\"", "");

		//图片宽度自适应
		content = content.replaceAll("<img ", "<img style='width:100%;' ");

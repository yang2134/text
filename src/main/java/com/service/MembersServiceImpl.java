...
public class MembersServiceImpl implements MembersService{
	...
	private Map<String, Object> getQueryMap(Members members,PageBean page){
		...
		if(members!=null){
			map.put("meid", members.getMeid());
			map.put("mname", members.getMname());
			map.put("photo", members.getPhoto());
			map.put("sname", members.getSname());
			map.put("tel", members.getTel());
			map.put("address", members.getAddress());
			map.put("qq", members.getQq());
			map.put("email", members.getEmail());
			map.put("matime", members.getMatime());
	...
	//根据ID删除
	public int deleteMembers(int id) throws Exception {
	...
	//根据ID得到对应的记录
	public Members queryMembersById(int id) throws Exception {
